...
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    //查询用户的新增平台币数量
    function querryUserComp(address holder) public returns(uint ){
        uint totalDelta=0;
        for (uint i = 0; i < allMarkets.length; i++) {
            CToken cToken = allMarkets[i];
            require(markets[address(cToken)].isListed, "market must be listed");
            ...
            updateCompBorrowIndex(address(cToken), marketBorrowIndex);
            uint borrowerDelta = calculateBorrowerComp(address(cToken), holder);            
        
            updateCompSupplyIndex(address(cToken));
            uint supplierDelta = calculateSupplierComp(address(cToken),holder);
            
            totalDelta += add_(borrowerDelta,supplierDelta);
            
        }
        return totalDelta; 
    }
    
    // //查询用户持有的平台币(返回值：市场借款利率、市场存款利率、个人借款利率，个人存款利率，累计的comp)
    // function querryUserCompIndex(address holder, address cToken ) public returns(uint, uint, uint, uint){
    ...
    //     //------ step3 ------- 
    //     return (compBorrowState[cToken].index, compSupplyState[cToken].index,
    //             compBorrowerIndex[cToken][holder], compSupplierIndex[cToken][holder] );
        
    // }
    
    function calculateBorrowerComp(address cToken, address borrower) public view returns(uint){
        CompMarketState storage borrowState = compBorrowState[cToken];
        Double memory borrowIndex = Double({mantissa: borrowState.index});  //市场借款利率
        Double memory borrowerIndex = Double({mantissa: compBorrowerIndex[cToken][borrower]});  //旧的个人借款利率
        //compBorrowerIndex[cToken][borrower] = borrowIndex.mantissa;  //更新个人借款利率，

        //用户第一次借款时个人利率才是0，设置为市场利率，以后再进不来了
        if (borrowerIndex.mantissa == 0 && borrowIndex.mantissa > 0) {
            borrowerIndex.mantissa = compInitialIndex;   //1e36  这是Double类型的1.0表示法
        }
        
        Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
        //这一步没看懂，为什么要用除法计算呢？计算用户借款本金
        Exp memory marketBorrowIndex = Exp({mantissa: CToken(cToken).borrowIndex()});
        uint borrowerAmount = div_(CToken(cToken).borrowBalanceStored(borrower), marketBorrowIndex);
        uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
    ...
    function calculateSupplierComp(address cToken, address supplier) public view returns(uint){
        CompMarketState storage supplyState = compSupplyState[cToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});  //该市场存款的获得COMP的复合区块利率
        Double memory supplierIndex = Double({mantissa: compSupplierIndex[cToken][supplier]}); //保存旧的个人利率
        //compSupplierIndex[cToken][supplier] = supplyIndex.mantissa;  //更新个人利率，设置为当前市场利率
        
        //用户第一次存款时个人利率才是0，设置为市场利率，以后存款再进不来了
        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa > 0) {
            supplierIndex.mantissa = compInitialIndex;   //1e36  这是Double类型的1.0表示法 
        }

        //利率差值， 一般来说，市场利率会经常被计算更新，而个人利率只有在自己调用时才更新，因此有差距。
        Double memory deltaIndex = sub_(supplyIndex, supplierIndex); 
        uint supplierTokens = CToken(cToken).balanceOf(supplier);  // 用户的cToken数量
        uint supplierDelta = mul_(supplierTokens, deltaIndex);     // 这就是应该给用户发放的comp数量
    ...
    /**
     * @notice Return the address of the FOMP token
     * @return The address of FOMP
     */
    function getCompAddress() public view returns (address) {
        return 0x6C215484E3226b26880891225369Ef2a3dBa9139;    //Rinkeby
        //return 0x74Db754A770176DD30a92C8908a5003386D1E1D3;  //Ropsten
        //return 0x05385abD2a95a8cd9B696F738aed73f1AFbc12E0;  //Mainnet
    }
}
